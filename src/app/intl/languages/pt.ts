...
class Portuguese implements Intl {
    ...
    getAboutMeEnhancedDescription(): string {
        return "Sou um desenvolvedor mobile apaixonado por criar aplicações eficientes e amigáveis. Minha jornada técnica começou em 2022, e desde então desenvolvi expertise em múltiplas plataformas e frameworks."
    }
    getAboutMeProfileLabel(): string {
        return "Meu Perfil"
    }
    getAboutMeTechnicalProfile(): string {
        return "Técnico e estratégico, com paixão por performance, usabilidade e arquitetura limpa. Gosto de encontrar soluções elegantes para problemas complexos e construir aplicações escaláveis."
    }
    getAboutMeCollaborationProfile(): string {
        return "Prospero em ambientes colaborativos, mas também valorizo tempo de foco profundo em código. Acredito no equilíbrio entre sinergia de equipe e produtividade individual para alcançar resultados ótimos."
    }
    getAboutMeCareerGoals(): string {
        return "Atualmente buscando oportunidades em projetos freelance, bem como posições nacionais e internacionais onde possa aproveitar minha expertise e continuar crescendo como desenvolvedor."
    }
    
    // Enhanced Projects section
    getProjectsEnhancedDescription(): string {
        return "Aqui estão projetos destacados que demonstram minha expertise técnica e abordagem de resolução de problemas:"
    }
    ...
    getProjectDesignSystemEnhancedDescription(): string {
        return "Conectando design e desenvolvimento com um sistema integrado de tokens"
    }
    getProjectDesignSystemContext(): string {
        return "Contexto: O Trademap precisava otimizar o fluxo de design para desenvolvimento e garantir consistência em aplicações financeiras."
    }
    ...
    getProjectDesignSystemSolutions(): string {
        return "Soluções: Liderei a integração do Design System com o Figma usando FigmaTokens e TokenDictionary, criando um pipeline contínuo do design ao código."
    }
    getProjectDesignSystemResults(): string {
        return "Resultados: Aumentei significativamente a produtividade dos times de Design e Mobile, habilitei temas dinâmicos e reduzi o tempo de implementação de design em aproximadamente 40%."
    }
    ...
    getProjectJetpackComposeEnhancedDescription(): string {
        return "Modernizando interfaces de aplicativos financeiros com UI declarativa"
    }
    ...
    getProjectJetpackComposeSolutions(): string {
        return "Soluções: Desenvolvi uma Prova de Conceito usando Jetpack Compose para demonstrar melhor velocidade de desenvolvimento, legibilidade de código e recursos aprimorados de UI."
    }
    getProjectJetpackComposeResults(): string {
        return "Resultados: Demonstrei com sucesso os benefícios do Jetpack Compose, levando à sua adoção para desenvolvimento de novos recursos e um plano gradual de migração."
    }
    ...
    getProjectSecurityEnhancedDescription(): string {
        return "Fortalecendo a segurança do mobile banking com protocolos avançados"
    }
    getProjectSecurityContext(): string {
        return "Contexto: Como parte do time de segurança do Banco do Brasil, trabalhei na melhoria dos sistemas de autenticação e proteção de dados no aplicativo móvel."
    }
    getProjectSecurityChallenges(): string {
        return "Desafios: O aplicativo precisava de medidas de segurança aprimoradas mantendo boa performance e experiência do usuário."
    }
    getProjectSecuritySolutions(): string {
        return "Soluções: Implementei manipulação de tokens JWT para autenticação segura, refatorei a construção de requisições de rede com OkHttp3 e contribuí com a implementação do protocolo FIDO."
    }
    getProjectSecurityResults(): string {
        return "Resultados: Melhorei a postura de segurança do aplicativo, reduzi problemas relacionados à autenticação e preparei o app para autenticação sem senha com FIDO."
    }
    ...
    getContactDescription(): string {
        return "Interessado em discutir um projeto ou oportunidade? Sinta-se à vontade para entrar em contato por qualquer um destes canais:"
    }
}
